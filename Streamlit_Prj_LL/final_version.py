...
data = pd.read_csv(f'{os.path.dirname(__file__)}\经济方面.csv'.replace('\\', '/'), encoding='gb18030')
...
image = Image.open(f'{os.path.dirname(__file__)}\LinearRegression().jpg'.replace('\\', '/'))
st.image(image, caption='LinearRegression')
# # 展示二级标题
st.subheader('3.2 使用Ridge进行预测')

image = Image.open(f'{os.path.dirname(__file__)}\Ridge().jpg'.replace('\\', '/'))
st.image(image, caption='Ridge')
# # 展示二级标题
st.subheader('3.3 使用Lasso进行预测')

image = Image.open(f'{os.path.dirname(__file__)}\Lasso().jpg'.replace('\\', '/'))
st.image(image, caption='Lasso')

# 展示一级标题
st.header('4. 对国内各省份生产总值GDP进行直观展示')
st.subheader('4.1 国内各省份GDP数据')
data = pd.read_csv(f'{os.path.dirname(__file__)}\province.csv'.replace('\\', '/'), encoding='utf8')
# print(data.head()) # 默认打出五行数据
st.dataframe(data)
