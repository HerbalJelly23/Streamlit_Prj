...
with st.expander('频率误差及波形失真度分析原理'):
    ...
    col1.markdown('- 这是由于对于DDS方法，使用条件往往是数字电路经过DAC产生波形，其谐波能量是难以计算的。')
    col1.markdown('- 且在更复杂的含有波形幅度挡位调节功能的系统中，THD分析难以具有代表性，计算量也大大提高。')
    col1.markdown('- 在此处使用波形面积的差作为失真度即标准差，更加符合DDS方法生成的波形失真度的语义。')
    col2.markdown(
